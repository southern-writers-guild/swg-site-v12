...
Step 4 confirmed 2026-08-28: real addresses are beau@southernwritersguild.com (Rick), hank@southernwritersguild.com (MJ Polk), gray@southernwritersguild.com (Grace) — mailboxes created and active. Independent Google Accounts for these three not yet created as of this date. **Correction, 2026-09-10:** this entry's claim that jp@southernwritersguild.com has its own independent Google Account was wrong — checked directly against a real Google sign-in attempt today, "Couldn't find this account." No such account exists. The YouTube Brand Account is actually owned by Rick's own personal Google account, confirmed directly by Rick. The Guild's real shared Drive folder (the one search actually finds, `*SHARED* SOUTHERN WRITERS GUILD`) is owned by Grace's personal Google account, not by any jp@ identity either — so the "shared Guild Drive (Step 5)" half of this claim was also never true as described. Whatever prompted the original 2026-08-28 log entry, it didn't result in a real, separate jp@ Google Account. Don't assume one exists anywhere else (Canva, Kit, etc.) without checking directly first.
...
- ~~`jp@southernwritersguild.com` is active with its own independent login, separate from Polk's main IONOS account.~~ **Corrected 2026-09-10 — false, see the note above.** `jp@` is a real, active mailbox on IONOS (see below), but there is no separate Google Account behind it. The YouTube Brand Account is owned by Rick's personal Google account instead.
...
1. Secure `jp@` access — password shared to MJ and Grace so all three can get in independently. **Mechanism superseded 2026-09-10** — see Step 7 below; no longer a vault.
...
4. Give each founder a real identity, free. IONOS already includes five mailbox slots; `jp@` uses one, each founder takes one of the remaining four (one spare left over) — a real, non-shared address on the domain, for mail. **Superseded 2026-09-10, confirmed directly by Rick: sign-in itself does not go through these addresses.** Each founder uses their own existing personal Google login for "Sign in with Google" everywhere — GitHub, Vercel, Sanity, Canva, and so on — not a new dedicated `@southernwritersguild.com` Google Account as this step originally planned. Real example already in production: MJ's Sanity membership shows as "Mikie Pdizzle," their personal account, not `hank@southernwritersguild.com`. **Fuller reasoning behind this not yet given — Rick said more explanation is coming when he has time. Treat "personal logins, not branded accounts" as settled; don't assume anything beyond that (e.g. whether the founder mailboxes still matter for anything besides receiving mail) until he explains further.**
...
7. Share the Guild's handful of shared-login passwords (`jp@` mail, Kit, Cloudflare) via a Google Family Group. **Superseded 2026-09-10, Rick's explicit call: no Proton Pass, no KeePass — both rejected as more than three people with not much going on need.** Real mechanism, confirmed current (2026-09): Google Password Manager now supports sharing an individual saved password with specific people in your Google Family Group (up to 6 members, don't need to be blood relatives, just invited by the family manager — Rick creates the group once, adds MJ and Grace by their personal Google accounts, the same ones they already sign into everything with). Sharing is per-password, not all-or-nothing — share just the 2-3 that are actually shared, nothing else. The recipient gets a real copy in their own Google Password Manager, ready to autofill — no new app, no new login, nothing to memorize. Requires Rick to actually save those passwords into his own Google Password Manager first, then use its Share action on each one — a credentials task, so it's his to do, not something this file's sessions touch. Full steps handed to Rick directly in chat 2026-09-10, not repeated here.
...
10. ~~Build the Creator Workstation~~ — **rejected 2026-09-10, Rick's explicit call: "we walked through that at length and decided it was redundant."** Not being built. In its place: the Guild's actual shared Google Drive folder (`*SHARED* SOUTHERN WRITERS GUILD`, owned by Grace, Rick already has access) holds `SWG_Where_Everything_Lives.md` (access/tools reference) and `SWG_Editing_The_Website.md` (the founder-facing Sanity editing manual, built 2026-09-10) — both moved into that folder this session specifically so MJ and Grace can actually reach them, since `Where_Everything_Lives` had been sitting unshared in Rick's personal Drive since it was created and neither founder could see it. That's the front door now: Sanity Studio directly for content, this shared folder for reference docs.
